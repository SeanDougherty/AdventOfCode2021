...
void turnOn(vector<Zone> &ons, Zone newOn) {
  vector<Zone> newLit = {};
  vector<Zone> intersecting = {};
  vector<Zone> newlyLitZones = {newOn};
  bool overlapExists = false;
  // Find any initial overlaps
  for (Zone on : ons) {
    if (overlap(on, newOn)) {
      intersecting.push_back(on);
      overlapExists = true;
    } else {
      newLit.push_back(on);
    }
  ...
  while (overlapExists) {
    //a list of newOns that potentiall overlap with intersecting
    //a list of intersectings that overlapped with the previous newOns
    //loop through intersecting
    overlapExists = false;
    for (Zone on : intersecting) {
      //if they overlap with a newOn
      ...
      for (Zone toAdd : newlyLitZones){
        ...
        // decompose the newOn with this intersection
        // add these decomposed newOns to a newer newOn list
        } else {
  ...
  for (Zone toAdd : intersecting) {
    newLit.push_back(toAdd);
  }
  for (Zone toAdd : newlyLitZones) {
    newLit.push_back(toAdd);
  }
