...
queue<TreeNode*> buildInput() {
  queue<TreeNode*> lines;
  ifstream input("input.txt");
  string buff;
  while (input.is_open()) {
    getline(input, buff);

    lines.push(buildTree(buff));
...
int main()
{
  queue<TreeNode*> rootsToSum = buildInput();

  TreeNode* root, *toExplode, *toSplit, *prev;
  root = rootsToSum.front();
  rootsToSum.pop();
  inOrder(root);
  cout << endl;
  cout << endl;
  bool isFound;
  while (!rootsToSum.empty()) {
    do {
    ...
    root = add(root, rootsToSum.front());
    inOrder(root);
    cout << endl;
    cout << endl;
    rootsToSum.pop();
  ...
  cout << endl;
  cout << endl;
  cout << fixed << magnitude(root)  << endl;
